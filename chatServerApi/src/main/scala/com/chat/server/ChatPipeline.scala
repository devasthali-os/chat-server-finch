...
object ChatPipeline {
  ...
  def pipeline(chatRequest: ChatRequest)(
      implicit executionContext: ExecutionContext): Future[ChatResponse] = {
    NluService.findIntent(chatRequest).map {
      case IntentResponse("NearbyCoffeeIntent") =>
        ChatResponse(chatRequest.correlationID,
